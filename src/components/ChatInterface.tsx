...
import { Send, Image, Sparkles, AlertCircle, AlertTriangle, X, Mic, MicOff } from 'lucide-react';
import { ChatMessage, DiaryEntry } from '../types';
import { generateAIResponse, analyzeEmotionWithAI } from '../utils/mockAI';
import { EmotionIndicator } from './EmotionIndicator';
import { InlineVoiceChat } from './InlineVoiceChat';
...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({ onGenerateEntry, currentEmotion }) => {
  ...
  return (
    <div className="h-full flex flex-col max-w-4xl mx-auto">
      ...
      <div className="card-header flex items-center justify-between bg-white/80 backdrop-blur-sm border-b border-gray-200 flex-shrink-0 fade-in-down">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={`btn-icon shadow-lg hover:shadow-xl hover-lift btn-press ${
              showVoiceChat 
            ...
            title={showVoiceChat ? "Close voice chat" : "Start voice conversation"}
          >
            {showVoiceChat ? <MicOff className="w-5 h-5" /> : <Mic className="w-5 h-5" />}
          </button>
        </div>
      </div>
      ...
      {showVoiceChat && (
        <div className="mx-6 mt-4 flex-shrink-0 fade-in-up">
          <InlineVoiceChat 
